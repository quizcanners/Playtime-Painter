    ...
    public class PainterMesh
    {
        ...
        public class Vertex : PainterSystemKeepUnrecognizedCfg
        {
            ...
            public bool SameUv(Vector2 uv, Vector2 uv1) =>
                (uv - GetUv(0)).magnitude < 0.0000001f && (uv1 - GetUv(1)).magnitude < 0.0000001f;
        ...
        public class Triangle : PainterSystemKeepUnrecognizedCfg
        {
            ...
            public override bool Decode(string tg, string data)
            {
                ...
                switch (tg)
                {
                    case "tex":
                        ...
                        break;
                    case "f0":
                        dominantCorner[0] = true;
                        break;
                    case "f1":
                        dominantCorner[1] = true;
                        break;
                    case "f2":
                        dominantCorner[2] = true;
                        break;
                    case "ew0":
                        edgeWeight[0] = data.ToFloat();
                        break;
                    case "ew1":
                        edgeWeight[1] = data.ToFloat();
                        break;
                    case "ew2":
            ...
            public bool Includes(LineData ld) =>
                (Includes(ld.points[0].meshPoint) && (Includes(ld.points[1].meshPoint)));
            ...
            public void AssignWeightedData(Vertex to, Vector3 weight)
            {
                ...
                var newV20 = vertexes[0].GetUv(0) * weight.x + vertexes[1].GetUv(0) * weight.y +
                             vertexes[2].GetUv(0) * weight.z;
                var newV21 = vertexes[0].GetUv(1) * weight.x + vertexes[1].GetUv(1) * weight.y +
                             vertexes[2].GetUv(1) * weight.z;
            ...
            {
                for (var i = 0; i < 3; i++)
                    if ((vertexes[i].meshPoint != l.points[0].meshPoint) &&
                        (vertexes[i].meshPoint != l.points[1].meshPoint))
                        return i;
        ...
        public class LineData : PainterSystem
        {
            ...
            public bool SameVertices(LineData other) => (((other.points[0].meshPoint == points[0].meshPoint) &&
                                                          (other.points[1].meshPoint == points[1].meshPoint)) ||
                                                         ((other.points[0].meshPoint == points[1].meshPoint) &&
                                                          (other.points[1].meshPoint == points[0].meshPoint)));
            ...
            public List<Triangle> GetAllTriangles()
            {
                ...
                foreach (var tri in uv.triangles)
                    ...
                    if (!allTriangles.Contains(tri) && tri.Includes(points[0].meshPoint) &&
                        tri.Includes(points[1].meshPoint))
                        allTriangles.Add(tri);
            ...
            public override bool Equals(object obj)
            {
                ...
                return (ld.points[0].meshPoint == points[0].meshPoint && ld.points[1].meshPoint == points[1].meshPoint)
                       || (ld.points[0].meshPoint == points[1].meshPoint &&
                           ld.points[1].meshPoint == points[0].meshPoint);
