        ...
        public override bool MouseEventPointedVertex()
        {
            if (PlaytimePainter_EditorInputManager.GetMouseButton(0))
            {
                if (_mergeUnMerge)
                {
                    ...
                    EditedMesh.Dirty |= PointedVertex.SetSmoothNormal(!PlaytimePainter_EditorInputManager.Shift);
            }
        ...
        public override bool MouseEventPointedLine()
        {
            if (PlaytimePainter_EditorInputManager.GetMouseButton(0))
            {
                if (_mergeUnMerge) {
                    ...
                    for (var i = 0; i < 2; i++)
                        Dirty |= PointedLine[i].SetSmoothNormal(!PlaytimePainter_EditorInputManager.Shift);
