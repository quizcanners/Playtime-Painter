...
namespace PlaytimePainter.MeshEditing
{
    ...
    public class MeshEditorManager : PainterClassCfg {
        ...
        public void UpdateLocalSpaceMousePosition()
        {
            ...
            onGridLocal = targetTransform.InverseTransformPoint(PlaytimePainter_GridNavigator.LatestMouseToGridProjection);
            collisionPosLocal = targetTransform.InverseTransformPoint(PlaytimePainter_GridNavigator.LatestMouseRaycastHit);
        }
        ...
        public void StopEditingMesh()
        {
            ...
            PlaytimePainter_GridNavigator.Instance.SetEnabled(false, false);
            UndoMoves.Clear();
            RedoMoves.Clear();
        }
        ...
        public void MoveVertexToGrid(PainterMesh.MeshPoint vp)
        {
            ...
            diff.Scale(PlaytimePainter_GridNavigator.Instance.GetGridPerpendicularVector());
            vp.localPos += diff;
        }
        ...
        public void AssignSelected(PainterMesh.Vertex newPnt)
        {
            ...
            {
                PlaytimePainter_GridNavigator.LatestMouseToGridProjection = SelectedUv.meshPoint.WorldPos;
        ...
        public PainterMesh.MeshPoint CreatePointAndFocus(Vector3 pos)
        {
            ...
            PlaytimePainter_GridNavigator.LatestMouseRaycastHit = pos;
        ...
        private bool RayCastVertexIsPointed()
        {
            ...
            if (alt)
                PlaytimePainter_GridNavigator.LatestMouseRaycastHit = PlaytimePainter_GridNavigator.LatestMouseToGridProjection;
            
            RaycastHit hit;
            var vertexIsPointed = false;

            if (PlaytimePainter_GridNavigator.RaycastMouse(out hit))
            {
                ...
                if (!alt)
                {
                    ...
                    if (vertexIsPointed)
                    {
                        PlaytimePainter_GridNavigator.LatestMouseRaycastHit = hit.transform.position;
                        UpdateLocalSpaceMousePosition();
                    ...
                    {
                        PlaytimePainter_GridNavigator.LatestMouseRaycastHit = hit.point;
                        UpdateLocalSpaceMousePosition();
                        editedMesh.SortAround(collisionPosLocal, true);
        ...
        public void DrowLinesAroundTargetPiece()
        {
            ...
            var projected = PlaytimePainter_GridNavigator.Instance.ProjectToGrid(piecePos); // piecePos * getGridMaskVector() + ptdPos.ToV3(false)*getGridPerpendicularVector();
            var gridMask = PlaytimePainter_GridNavigator.Instance.GetGridMaskVector() * 128 + projected;
