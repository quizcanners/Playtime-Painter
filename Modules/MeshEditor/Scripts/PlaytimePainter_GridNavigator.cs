...
#pragma warning disable IDE0018 // Inline variable declaration
    ...
    [ExecuteInEditMode]
    public class PlaytimePainter_GridNavigator : PainterSystemMono
    {
        public static PlaytimePainter_GridNavigator Instance
        {
            get
            {
                ...
                if (!ApplicationIsQuitting)
                {
                    _inst = PainterCamera.Inst
                        .GetComponentInChildren<PlaytimePainter_GridNavigator>(); //(GridNavigator)FindObjectOfType<GridNavigator>();
                    if (_inst) return _inst;
                    try
                    {
                        var prefab = Resources.Load(PainterDataAndConfig.PREFABS_RESOURCE_FOLDER + "/grid") as GameObject;
                        _inst = Instantiate(prefab).GetComponent<PlaytimePainter_GridNavigator>();
        ...
        private static PlaytimePainter_GridNavigator _inst;
