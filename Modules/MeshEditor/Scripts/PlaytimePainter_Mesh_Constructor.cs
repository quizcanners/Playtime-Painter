    ...
    public class MeshConstructor {
        ...
        public Vector4[] ShadowBake {
            get {
                ...
                foreach (PainterMesh.MeshPoint point in edMesh.meshPoints)
                foreach (PainterMesh.Vertex vert in point.vertices)
                    _shadowBake[vert] = point.shadowBake;
                return _shadowBake;
            }
        }
        ...
        public Vector4[] EdgeData 
        {
            get 
            {
                ...
                foreach (var tri in edMesh.triangles) 
                {
                    for (var no = 0; no < 3; no++) 
                    {
                        PainterMesh.Vertex vertex = tri.vertexes[no];
                        float edge = (vertex.triangles.Count == 1) ? 1 : 0;
                        _edgeData[vertex] = new Vector4(
                            x: no == 0 ? 0 : edge,
                            y: no == 1 ? 0 : edge,
                            z: no == 2 ? 0 : edge,
                            w: vertex.meshPoint.edgeStrength.x);
                    }
                }
                return _edgeData;
            }
        ...
        public Vector4[] SeamData
        {
            get
            {
                ...
                _seamData = new Vector4[vertexCount];

                foreach (PainterMesh.MeshPoint point in edMesh.meshPoints)
                    foreach (PainterMesh.Vertex vert in point.vertices)
                        _seamData[vert] = vert.meshPoint.edgeStrength;
                ...
                foreach (var tri in edMesh.triangles)
                    for (var no = 0; no < 3; no++)
                    {
                        ...
                        if (weight.magnitude < 0.3f)
                            weight[(no + 1) % 3] = up.meshPoint.edgeStrength.x;
        ...
        public MeshConstructor(PainterComponent painter)
        {
            profile = painter.MeshProfile;
            edMesh = new MeshData(painter);
            mesh = painter.SharedMesh;
            if (!mesh)
                mesh = new Mesh();
        }

        internal MeshConstructor(MeshData edMesh, MeshPackagingProfile solution, Mesh freshMesh)
        {
            profile = solution;
            this.edMesh = edMesh;
            mesh = freshMesh;
        ...
        public Mesh Construct(PainterComponent p)
        {
            var m = Construct();
