...
namespace Painter {
    ...
    public static class PainterPEGI_Extensions {
        ...
        public static bool Mode_Type_PEGI(this BrushConfig brush, bool cpuBlit) {
            ...
            var bm = brush.blitMode;

            changed |= pegi.select(ref brush._bliTMode, BlitMode.allModes.ToArray(), true);
            ...
            if (!cpuBlit) {
                ...
                changed |= pegi.select<BrushType>(ref brush._type, BrushType.allTypes);

                changed |= brush.type.PEGI(brush);
            }
        ...
        public static bool BrushForTargets_PEGI(this BrushConfig brush) {
            ...
            if ((brush.TargetIsTex2D ? icon.CPU : icon.GPU).Click(
                brush.TargetIsTex2D ? "Render Texture Config" : "Texture2D Config", 45)) {
                brush.TargetIsTex2D = !brush.TargetIsTex2D;
                brush.setSupportedFor(brush.TargetIsTex2D, true);
                changed = true;
            }

            bool smooth = brush._type != BrushTypePixel.inst.index;

            if ((brush.TargetIsTex2D) &&  pegi.toggle(ref smooth, icon.Round.getIcon(), icon.Square.getIcon(), "Smooth/Pixels Brush", 45))
            {
                changed = true;
                brush._type =  smooth ? BrushTypeNormal.inst.index : BrushTypePixel.inst.index;
            }
        ...
        public static bool BrushParameters_PEGI(this BrushConfig brush, PlaytimePainter painter) {
            ...
            if ((PainterManager.GotBuffers() || (id.renderTexture != null)) && (id.texture2D != null)) {
                if ((cpuBlit ? icon.CPU : icon.GPU).Click(
                    ...
                    brush.setSupportedFor(cpuBlit, id.renderTexture == null);
                    changed = true;
                }
            }
            ...
            if (brush.Mode_Type_PEGI(cpuBlit)) {
                if (brush.type  == BrushTypeDecal.inst)  
