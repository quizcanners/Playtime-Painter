...
Shader "Painter_Experimental/PixArt/Outline" {
	Properties{
...
//	float4 nn = tex2Dlod(_Bump, float4(i.texcoord.xy* _MainTex_TexelSize.z + float2(-hold.x, -hold.y), 0, 0));
	...
	float4 col = tex2Dlod(_MainTex, float4(pixuv, 0, 0))*(1-wid);//((color)*wid + c*(1 - wid));
