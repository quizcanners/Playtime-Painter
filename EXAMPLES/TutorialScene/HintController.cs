...
public class HintController : MonoBehaviour {
    ...
    void Update() {
        ...
        switch (stage) {
            case HintStage.enableTool:
                if (PlaytimePainter.IsCurrent_Tool) { SetStage(HintStage.draw); timer = 3f; } break;
            case HintStage.draw:
                if (!PlaytimePainter.IsCurrent_Tool) { SetStage(HintStage.enableTool); break; } if (timer < 0) { SetStage(HintStage.addTool); } break;
            case HintStage.addTool:
                if (picture.GetComponent<PlaytimePainter>() != null) { SetStage(HintStage.addTexture); } break;
            case HintStage.addTexture:
