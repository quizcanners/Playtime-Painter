...
// With Trees you can create class independent methods. 
// With them user will not need to populate animation with many functions. Behaviours will be class independent.
// They will be components or prefabs that get attached when are called. This way logic system can start real time behaviors.
// 1: Unilink Variables Veb
// 2: Make possible for StoryTells to provıde Ulınk of parameter so that event could provıde ıt back on event.
// 3: 

// Instead of serialization, make them StoryLinkable Serializable.
