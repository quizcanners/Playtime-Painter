    ...
    public partial class PainterComponent 
    {
        ...
        internal static class Documentation 
        {
            ...
            public static void Inspect()
            {
                ...
                {
                    if ("About".PegiLabel().IsEntered().Nl())
                    {
                    ...
                    if ("How to Use Painter".PegiLabel().IsEntered().Nl())
                    {
                        using (_context_HowToPaint.StartContext())
                        {
                            if ("Painter Component".PegiLabel().IsEntered().Nl())
                            {
                            ...
                            {
                                "{0} will be automatically created in the scene to help manage all the painting processes.".F(nameof(Singleton_PainterCamera)).PegiLabel().WriteBig();
                            }
                            ...
                            if ("Stroke".PegiLabel().IsEntered().Nl())
                            {
                                ("{0} is class that contains coordinates of where to paint. It would be World Space coordinates for Sphere Brush, and UV-space coordinates for other brushes." +
                                    ...
                                    .F(nameof(Stroke)).PegiLabel().WriteBig();
                            }
                        }
                    }

                    if ("FAQ".PegiLabel().IsEntered().Nl())
                    {
                        using (_context_FAQ.StartContext())
                        {
                            if ("Can I integrate Painter into my game?".PegiLabel().IsEntered().Nl())
                            {
                                (" There is no reason why you can't. This asset doesn't contain or depend on any plugins and does everything using Unity functions. " + Environment.NewLine +
                                 ...
                                 " interface. It should show warnings/hints when something needs to be set up. ").PegiLabel().WriteBig();
                            }

                            if ("How do I Save/Load/Undo changes to textures?".PegiLabel().IsEntered().Nl())
                            {
                                ("Undo/Redo needs to be enabled per texture in {0}. Otherwise there are just too many scenarios when lots of memory will be used due to unwanted backups and redo steps.".F(MsgPainter.TextureSettings.GetText()) +
                                  ...
                                  "load from it. If not saved, any changes to the texture will be lost once Unity is restarted, or texture reimported.").PegiLabel().WriteBig();
                            }

                            if ("What should I be careful about?".PegiLabel().IsEntered().Nl())
                            {
                                ("I tried to forsee as many scenarious as possible. So deleting something shouldn't be a problem for Playtime Painter." +
                                 ...
                                 " support email already typed in, so don't shy away from reporting those nasty bugs. See something, say something.").PegiLabel().WriteBig();
                            }
                        }
                    }

                    if ("Performance/Quality tips".PegiLabel().IsEntered().Nl())
                    {
                        using (_context_PerfTip.StartContext())
                        {
                            if ("Slower in Editor in Android/iOS mode".PegiLabel().IsEntered().Nl())
                            {
                                "If Editor is set for Android/iOS then Unity will try to emulate that API and as a result, painting will be slower.".PegiLabel()
                                    .WriteBig();
                            }

                            if ("Use GPU brush, big brush for CPU is slow.".PegiLabel().IsEntered().Nl())
                            {
                                "GPU brush is always faster for every task. CPU brush uses code written in C#, so it is much easier to add your own blit modes to it, but it gets slower the bigger the brush gets.".PegiLabel().WriteBig();
                            }

                            if ("Sphere brush is the best".PegiLabel().IsEntered().Nl())
                            {
                                (" Sphere brush is the best for editing complex models. It uses world space instead of texture space so the result is often more as one would expect." +
                                  ...
                                  " undoubtfully come with performance cost (when used)").PegiLabel().WriteBig();
                            }

                            if ("Fold In Material".PegiLabel().IsEntered().Nl())
                            {
                    ...
                    if ("Good to know, release notes".PegiLabel().IsEntered().Nl())
                    {
                        using (_context_Useful.StartContext())
                        {
                            if (_context_Useful.IsAnyEntered == false)
                            {
                                ("This section will most often relate to various findings related to Unity engine. I will also list all nuances and issues that may relate" +
                                    " to current version only in most cases. ").PegiLabel().WriteBig();
                            }

                            if ("MSAA + HDR results in one frame delay".PegiLabel().IsEntered().Nl())
                            {
                    ...
                    if ("Useful Links".PegiLabel().IsEntered().Nl())
                    {
                        " Something I often use when writing shaders. ".PegiLabel().WriteBig();

                        if ("Simple circle shader template".PegiLabel().ClickText(15).Nl())
                            Application.OpenURL("https://gist.github.com/quizcanners/b90d2644d7b990d0574307218478383a");

                        if ("Shader Cheat-Sheet".PegiLabel().ClickText(15).Nl())
                            Application.OpenURL("https://gist.github.com/quizcanners/0da1cbad4b1e2187af73f6ab52a6dabb");

                        if ("Shader commands".PegiLabel().ClickText(15).Nl())
